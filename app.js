...
function saveLocalState(st){localStorage.setItem(localKey(),JSON.stringify(st));}
function getLocal(){const st=localState();return {accounts:Array.isArray(st.accounts)?st.accounts:[],cards:Array.isArray(st.cards)?st.cards:[],recurrences:Array.isArray(st.recurrences)?st.recurrences:[],cardPurchases:Array.isArray(st.cardPurchases)?st.cardPurchases:[]}}
...
function renderLocalModules(){
 ...
 $('#accountsTotal').textContent=localMoney(accountTotal);$('#cardsLimitTotal').textContent=localMoney(limitTotal);$('#cardsBillsTotal').textContent=localMoney(bills);$('#recurrenceCount').textContent=st.recurrences.length;
 $('#accountsList').innerHTML=st.accounts.length?st.accounts.map(a=>`<article class="local-card account-card"><div class="local-card-head"><span class="local-icon">${a.kind==='poupanca'?'🐷':a.kind==='dinheiro'?'💵':'🏦'}</span><div><b>${esc(a.name)}</b><small>${esc(a.kindLabel||'Conta')}</small></div></div><strong>${localMoney(a.balance)}</strong><div class="local-actions"><button onclick="editAccount('${escAttr(a.id)}')">✏️ Editar</button><button class="danger" onclick="deleteAccount('${escAttr(a.id)}')">Excluir</button></div></article>`).join(''):'<div class="empty">Nenhuma conta cadastrada. Adicione sua conta corrente, poupança ou dinheiro em espécie.</div>';
 $('#cardsList').innerHTML=st.cards.length?st.cards.map(c=>{
   const inst=st.cardPurchases.map(x=>({x,inst:installmentForMonth(x,currentMonth)})).filter(v=>v.x.cardId===c.id&&v.inst); const bill=inst.reduce((a,v)=>a+v.inst.amount,0); const avail=Math.max(0,Number(c.limit||0)-bill);
   return `<article class="local-card credit-card"><div class="credit-top"><span>${esc(c.name)}</span><b>💳</b></div><div class="credit-number">•••• ${esc(c.last4||'0000')}</div><div class="credit-values"><div><small>Limite</small><strong>${localMoney(c.limit)}</strong></div><div><small>Fatura atual</small><strong>${localMoney(bill)}</strong></div></div><div class="limit-bar"><span style="width:${Math.min(100,Number(c.limit)?bill/Number(c.limit)*100:0)}%"></span></div><small class="limit-help">Disponível: ${localMoney(avail)} · Vencimento dia ${esc(c.dueDay||'—')}${c.closeDay?' · Fecha dia '+esc(c.closeDay):''}</small><div class="local-actions"><button onclick="addCardPurchase('${escAttr(c.id)}')">＋ Compra</button><button onclick="editCard('${escAttr(c.id)}')">✏️ Editar</button><button class="danger" onclick="deleteCard('${escAttr(c.id)}')">Excluir</button></div>${inst.length?`<div class="purchase-list">${inst.slice(0,6).map(v=>`<div><span>${esc(v.x.description)}${Number(v.x.installments||1)>1?` · ${v.inst.number}/${v.x.installments}`:''}</span><b>${localMoney(v.inst.amount)}</b></div>`).join('')}</div>`:''}</article>`;
 }).join(''):'<div class="empty">Nenhum cartão cadastrado. Cadastre um cartão para acompanhar limite e fatura.</div>';
 $('#recurrencesList').innerHTML=st.recurrences.length?st.recurrences.map(r=>`<div class="row local-row"><div><b>${esc(r.description)}</b><small>${r.type==='income'?'Receita':'Despesa'} · ${localMoney(r.amount)} · ${frequencyLabel(r.frequency)} · próximo: ${dateBR(r.nextDate)||'—'}</small></div><div class="local-actions"><button onclick="generateRecurrence('${escAttr(r.id)}')">＋ Gerar</button><button onclick="editRecurrence('${escAttr(r.id)}')">✏️</button><button class="danger" onclick="deleteRecurrence('${escAttr(r.id)}')">🗑️</button></div></div>`).join(''):'<div class="empty">Nenhuma recorrência cadastrada. Exemplos: aluguel, internet, salário ou assinatura.</div>';
}
...
function openLocalEdit(type,obj){
 if(type==='account'){localModal(obj?'Editar conta':'Nova conta',`<form id="localAccountForm"><label>Nome</label><input name="name" required value="${escAttr(obj?.name||'')}"><label>Tipo</label><select name="kind"><option value="corrente" ${obj?.kind==='corrente'?'selected':''}>Conta corrente</option><option value="poupanca" ${obj?.kind==='poupanca'?'selected':''}>Poupança</option><option value="dinheiro" ${obj?.kind==='dinheiro'?'selected':''}>Dinheiro</option><option value="outro" ${obj?.kind==='outro'?'selected':''}>Outra</option></select><label>Saldo atual</label><input name="balance" type="number" step="0.01" value="${Number(obj?.balance||0)}"><button class="primary">Salvar conta</button></form>`);$('#localAccountForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),st=getLocal();const item={id:obj?.id||crypto.randomUUID(),name:f.get('name'),kind:f.get('kind'),kindLabel:{corrente:'Conta corrente',poupanca:'Poupança',dinheiro:'Dinheiro',outro:'Outra'}[f.get('kind')],balance:Number(f.get('balance')||0)};if(obj)st.accounts=st.accounts.map(x=>x.id===obj.id?item:x);else st.accounts.unshift(item);saveLocalState(st);closeModal();renderLocalModules()}}
