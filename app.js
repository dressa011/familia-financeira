...
function txRowFull(x){const paid=x.status==='paid';return `<div class="row tx-row"><div><b>${esc(x.description)}</b><small>${x.type==='income'?'Receita':'Despesa'} · ${dateBR(x.due_date)} · ${paid?'Pago':'Pendente'}${x.profiles?.full_name?' · '+esc(x.profiles.full_name):''}</small></div><div class="row-actions tx-actions"><span class="amount ${x.type}">${x.type==='expense'?'-':''}${money(x.amount)}</span>${x.type==='expense'&&!paid?`<button class="pay-btn" onclick="payTx('${x.id}')">✓ Pagar</button>`:''}<button class="doc-btn" onclick="manageDocs('${x.id}')">📎 PDF</button><button class="icon-btn" onclick="deleteTx('${x.id}')">🗑️</button></div></div>`}
...
function openModal(type){let title='',html='';if(type==='transaction'){title='Novo lançamento';html=`<form id="txForm"><label>Tipo</label><select name="type"><option value="expense">Despesa</option><option value="income">Receita</option></select><label>Descrição</label><input name="description" required placeholder="Ex.: Arroz"><label>Valor</label><input name="amount" type="number" step="0.01" min="0" required><label>Vencimento</label><input name="due_date" type="date"><label>Status</label><select name="status"><option value="pending">Pendente</option><option value="paid">Pago</option></select><button class="primary">Salvar lançamento</button></form>`}else{title='Nova meta';html=`<form id="goalForm"><label>Nome</label><input name="name" required placeholder="Ex.: Reserva de emergência"><label>Valor da meta</label><input name="target_amount" type="number" step="0.01" min="0.01" required><label>Valor já guardado</label><input name="current_amount" type="number" step="0.01" min="0" value="0"><label>Prazo</label><input name="deadline" type="date"><label>Descrição</label><textarea name="description" rows="3"></textarea><button class="primary">Salvar meta</button></form>`}$('#modalContent').innerHTML=`<h3>${title}</h3>${html}`;$('#modal').classList.remove('hidden');$('#txForm')?.addEventListener('submit',saveTransaction);$('#goalForm')?.addEventListener('submit',saveGoal)}
async function payTx(id){if(!confirm('Marcar esta despesa como PAGA? Ela não será apagada.'))return;const {error}=await db.from('transactions').update({status:'paid',paid_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',id);if(error)alert(error.message);else await loadAll()}
async function manageDocs(id){const {data,error}=await db.from('documents').select('id,file_name,storage_path,created_at').eq('transaction_id',id).order('created_at',{ascending:false});if(error){alert('Não foi possível carregar os PDFs: '+error.message);return;}let html=`<div class="docs-list">${(data||[]).map(d=>{const url=db.storage.from('documents').getPublicUrl(d.storage_path).data.publicUrl;return `<div class="doc-item"><span>📄 ${esc(d.file_name)}</span><a href="${url}" target="_blank" rel="noopener">Abrir</a></div>`}).join('')||'<div class="empty">Nenhum PDF anexado.</div>'}</div><form id="docForm"><label>Anexar comprovante / PDF</label><input id="docFile" type="file" accept="application/pdf,.pdf" required><button class="primary">Enviar PDF</button></form><small>O PDF ficará vinculado a esta despesa.</small>`;$('#modalContent').innerHTML=`<h3>Comprovantes da despesa</h3>${html}`;$('#modal').classList.remove('hidden');$('#docForm').addEventListener('submit',e=>uploadDoc(e,id))}
async function uploadDoc(e,transactionId){e.preventDefault();const file=$('#docFile').files[0];if(!file)return;if(file.type!=='application/pdf')return alert('Selecione somente um arquivo PDF.');const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');const path=`${me.id}/${transactionId}/${crypto.randomUUID()}-${safe}`;const up=await db.storage.from('documents').upload(path,file,{contentType:'application/pdf',upsert:false});if(up.error){alert('Falha ao enviar PDF. Verifique se existe um bucket público chamado “documents” no Storage do Supabase.');return;}const ins=await db.from('documents').insert({transaction_id:transactionId,file_name:file.name,storage_path:path,uploaded_by:me.id});if(ins.error){await db.storage.from('documents').remove([path]);alert(ins.error.message);return;}await manageDocs(transactionId);}
