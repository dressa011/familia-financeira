...
function txRowFull(x){return `<div class="row"><div><b>${esc(x.description)}</b><small>${x.type==='income'?'Receita':'Despesa'} · ${dateBR(x.due_date)} · ${esc(x.profiles?.full_name||'')}</small></div><div class="row-actions"><span class="amount ${x.type}">${x.type==='expense'?'-':''}${money(x.amount)}</span><button class="icon-btn" onclick="deleteTx('${x.id}')">🗑️</button></div></div>`}
