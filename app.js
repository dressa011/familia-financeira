...
window.deleteTx=async id=>{if(!confirm('Excluir este lançamento?'))return;const {error}=await db.from('transactions').delete().eq('id',id).eq('created_by',me.id);if(error)alert(error.message);else await loadAll()};
