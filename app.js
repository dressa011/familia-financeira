...
async function loadAll(){
  ...
  const results = await Promise.all([
    db.from('transactions').select('id,type,amount,description,status,due_date,paid_at,created_at,created_by,responsible_profile_id,account_id,card_id,card_purchase_id,recurrence_id,installment_number,installments').order('due_date',{ascending:true,nullsFirst:false}),
    ...
    db.from('card_purchases').select('*').order('purchase_date',{ascending:false}),
    db.from('recurrences').select('*').order('next_date',{ascending:true}),
    profile?.role === 'admin'
